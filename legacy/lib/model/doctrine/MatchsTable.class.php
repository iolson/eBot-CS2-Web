...
class MatchsTable extends Doctrine_Table
{
    ...
    {
        return $this->createQuery()->where('status >= ? AND status <= ?', [Matchs::STATUS_NOT_STARTED, Matchs::STATUS_END_MATCH])->orderBy('status ASC');
    }

    public function getZoomMatch()
    {
        return $this->createQuery()
            ->where('status >= ? AND status <= ?', [Matchs::STATUS_FIRST_SIDE, Matchs::STATUS_OT_SECOND_SIDE])
            ->orderBy('RAND()')
            ->fetchOne();
    }
    ...
    {
        return $this->createQuery()
            ->where('status >= ? AND status <= ?', [Matchs::STATUS_WU_KNIFE, Matchs::STATUS_END_MATCH])
            ->orderBy('RAND()')
            ->fetchOne();
    }
