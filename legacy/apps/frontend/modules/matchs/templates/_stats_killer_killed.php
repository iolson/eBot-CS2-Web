            ...
            <?php foreach ($match->getMap()->getPlayer() as $player) { ?>
                ...
                <td style="width: 30px; text-align: center;">
                    <div class="progress progress-striped" style="width: 25px; height: 25px;  margin-bottom: 0px; margin: auto; ">
                        <div class="bar <?php if ($player->getTeam() == 'b') {
                            echo 'bar-danger';
        ...
        <?php foreach ($match->getMap()->getPlayer() as $player) { ?>
            ...
            <tr>
                <td style="width: 250px; min-width: 250px;" id="player-<?php echo $player->getId(); ?>">
                    ...
                    <a style="font-size: 11px" href="<?php echo url_for('player_stats', ['id' => $player->getSteamid()]); ?>"><?php echo $player->getPseudo(); ?></a>
                </td>
                <?php foreach ($match->getMap()->getPlayer() as $player2) { ?>
                    <?php if ($player2->getTeam() == 'other') {
                        continue;
                    } ?>
                    <td class="highlight_name<?php if (@$players[$player->getId()][$player2->getId()] * 1 == 0) {
                        echo ' muted';
                    } ?>" player="<?php echo $player->getId(); ?>" target="<?php echo $player2->getId(); ?>" style="text-align: center;"><?php echo @$players[$player->getId()][$player2->getId()] * 1; ?></td>
                <?php } ?>
            </tr>
        <?php } ?>
    </tbody>
