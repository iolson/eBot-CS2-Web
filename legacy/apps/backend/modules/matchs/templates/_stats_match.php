...
foreach ($rounds as $round) {
    ...
    if ($last == $win) {
        ...
        $data[++$i] = ['type' => $win, 'value' => 1];
        $round_current = 1;
    }

    $count++;
    if ($count == $match->getMaxRound()) {
        $data[++$i] = ['type' => 'seperator', 'value' => 1];
        $i++;
    }
    $last = $win;
}
    ...
    <tr>
                            ...
                            <?php foreach ($data as $d) { ?>
                                ...
                                <?php } else { ?>
                                    <div <?php if ($d['value'] > 1) {
                                        $class = 'needTips'; ?>title="<?php echo $d['value']; ?> <?php echo __('rounds'); ?>"<?php } ?> class="bar <?php echo ($d['type'] == 'ct') ? '' : 'bar-danger'; ?> <?php echo $class; ?>" style="width: <?php echo $size * $d['value']; ?>px;"></div>
                                <?php } ?>
                            <?php } ?>
                        </div>
        ...
        <td width="50%" valign="top">
            <table class="table table-striped table-condensed">
                <tr>
                    <th width="200"></th>
                    <th><?php echo $match->getTeamA()->exists() ? $match->getTeamA() : $match->getTeamAName(); ?> <?php if ($match->getScoreA() > $match->getScoreB()) { ?><i class="icon-star"></i><?php } ?></th>
                    <th><?php echo $match->getTeamB()->exists() ? $match->getTeamB() : $match->getTeamBName(); ?> <?php if ($match->getScoreB() > $match->getScoreA()) { ?><i class="icon-star"></i><?php } ?></th>
                </tr>
                <tr>
                    <th width="200"><?php echo __('Victory: Bomb Exploded'); ?></th>
                    <td><?php echo $stats['team_a']['bomb_planted_win']; ?> <?php if ($stats['team_a']['bomb_planted_win'] > $stats['team_b']['bomb_planted_win']) { ?><i class="icon-star"></i><?php } ?></td>
                    <td><?php echo $stats['team_b']['bomb_planted_win']; ?> <?php if ($stats['team_b']['bomb_planted_win'] > $stats['team_a']['bomb_planted_win']) { ?><i class="icon-star"></i><?php } ?></td>
                </tr>
                <tr>
                    <th width="200"><?php echo __('Defeat: Bomb Defused'); ?></th>
                    <td><?php echo $stats['team_a']['bomb_planted_loose']; ?> <?php if ($stats['team_a']['bomb_planted_loose'] > $stats['team_b']['bomb_planted_loose']) { ?><i class="icon-star"></i><?php } ?></td>
                    <td><?php echo $stats['team_b']['bomb_planted_loose']; ?> <?php if ($stats['team_b']['bomb_planted_loose'] > $stats['team_a']['bomb_planted_loose']) { ?><i class="icon-star"></i><?php } ?></td>
                </tr>
                <tr>
                    <th width="200"><?php echo __('Victory: Bomb Defused'); ?></th>
                    <td><?php echo $stats['team_a']['bomb_defused']; ?> <?php if ($stats['team_a']['bomb_defused'] > $stats['team_b']['bomb_defused']) { ?><i class="icon-star"></i><?php } ?></td>
                    <td><?php echo $stats['team_b']['bomb_defused']; ?> <?php if ($stats['team_b']['bomb_defused'] > $stats['team_a']['bomb_defused']) { ?><i class="icon-star"></i><?php } ?></td>
                </tr>
                <tr>
                    <th width="200"><?php echo __('Victory'); ?></th>
                    <td><?php echo $stats['team_a']['bomb_explosed']; ?> <?php if ($stats['team_a']['bomb_explosed'] > $stats['team_b']['bomb_explosed']) { ?><i class="icon-star"></i><?php } ?></td>
                    <td><?php echo $stats['team_b']['bomb_explosed']; ?> <?php if ($stats['team_b']['bomb_explosed'] > $stats['team_a']['bomb_explosed']) { ?><i class="icon-star"></i><?php } ?></td>
                </tr>
                <tr>
                    <th width="200"><?php echo __('Total Elimination'); ?></th>
                    <td><?php echo $stats['team_a']['kill']; ?> <?php if ($stats['team_a']['kill'] > $stats['team_b']['kill']) { ?><i class="icon-star"></i><?php } ?></td>
                    <td><?php echo $stats['team_b']['kill']; ?> <?php if ($stats['team_b']['kill'] > $stats['team_a']['kill']) { ?><i class="icon-star"></i><?php } ?></td>
                </tr>
                <tr>
                    <th width="200"><?php echo __('Victory: Out of Time'); ?></th>
                    <td><?php echo $stats['team_a']['time']; ?> <?php if ($stats['team_a']['time'] > $stats['team_b']['time']) { ?><i class="icon-star"></i><?php } ?></td>
                    <td><?php echo $stats['team_b']['time']; ?> <?php if ($stats['team_b']['time'] > $stats['team_a']['time']) { ?><i class="icon-star"></i><?php } ?></td>
                </tr>
                <tr>
                    <th width="200"><?php echo __('Longest Round Streak'); ?></th>
                    <td>
                        <?php echo $round_a; ?> <?php if ($round_a > $round_b) { ?><i class="icon-star"></i><?php } ?>
                    </td>
                    <td>
                        <?php echo $round_b; ?> <?php if ($round_b > $round_a) { ?><i class="icon-star"></i><?php } ?>
                    </td>
                </tr>
            </table>
        </td>
            ...
            <div class="carousel-inner">
                ...
                <?php foreach ($rounds as $round) { ?>
                    ...
                    foreach ($events as $event) {
                        ...
                        $eventArray[] = ['type' => $event->getEventName(), 'time' => $event->getEventTime(), 'color' => $color, 'text' => '<b>'.$event->getEventTime().'s</b> : '.$text];
                    }
                    ?>
                    ...
                    ?>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <?php echo image_tag('/images/icons/flag_'.($round->bomb_planted ? 'green' : 'red').'.png', ['class' => 'needTips', 'title' => ($round->bomb_planted ? 'oui' : 'non')]); ?>
                                                </td>
                                            </tr>
                                            <tr>
                                                <th width="200"><?php echo __('Bombe defused'); ?></th>
                                                <td>
                                                    <?php echo image_tag('/images/icons/flag_'.($round->bomb_defused ? 'green' : 'red').'.png', ['class' => 'needTips', 'title' => ($round->bomb_defused ? 'oui' : 'non')]); ?>
                                                </td>
                                            </tr>
                                            <tr>
                                                <th width="200"><?php echo __('Bombe exploded'); ?></th>
                                                <td>
                                                    <?php echo image_tag('/images/icons/flag_'.($round->bomb_exploded ? 'green' : 'red').'.png', ['class' => 'needTips', 'title' => ($round->bomb_exploded ? 'oui' : 'non')]); ?>
                                                </td>
                                            </tr>
                        ...
                        if ($kill->getKillerTeam() == 'CT') {
                            ...
                            $color = 'black';
                                                ...
                                                ?>
                                                    ...
                                                    <td width="100">
                                                        <?php echo image_tag('/images/kills/csgo/'.$kill->getWeapon(), ['class' => 'needTips', 'title' => $kill->getWeapon()]); ?>
