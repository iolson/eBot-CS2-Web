...
<?php } else { ?>
    ...
    <form class="form-horizontal" action="<?php echo $_SERVER['PHP_SELF'] ?>?step=3" method="POST">
        ...
        <div class="control-group">
            ...
            <div class="controls">
                <input type="text" id="log" name="log" value="<?php if (! empty($_POST['log'])) {
                    echo $_POST['log'];
                } ?>" placeholder="e.g. /home/ebot/eBot-CSGO/logs/log_match" class="input-xxlarge">
            </div>
        </div>
        <div class="control-group">
            <label class="control-label" for="admin_log">Full Admin-Match-Log Path</label>
            <div class="controls">
                <input type="text" id="admin_log" name="admin_log" value="<?php if (! empty($_POST['admin_log'])) {
                    echo $_POST['admin_log'];
                } ?>" placeholder="e.g. /home/ebot/eBot-CSGO/logs/log_match_admin" class="input-xxlarge">
            </div>
        </div>
        <legend>Demos</legend>
        <div class="control-group">
            ...
            <div class="controls">
                <input type="text" id="demo" name="demo" value="<?php if (! empty($_POST['demo'])) {
                    echo $_POST['demo'];
                } ?>" placeholder="e.g. /home/ebot/eBot-CSGO/demos" class="input-xxlarge">
            </div>
        </div>
        <div class="control-group">
            <label class="control-label" for="demoDownload">Make Demos downloadable?</label>
            <div class="controls">
                <input type="checkbox" id="demoDownload" value="true" name="demoDownload" <?php if (! empty($_POST['demoDownload'])) {
        ...
        <div class="control-group">
            ...
            <div class="controls">
                <input type="text" id="ip" name="ip" value="<?php if (! empty($_POST['ip'])) {
                    echo $_POST['ip'];
                } ?>" placeholder="e.g. 192.168.0.1" class="input-xlarge">&nbsp;&#58;&nbsp;
                <input type="text" id="port" name="port" value="<?php if (! empty($_POST['port'])) {
                    echo $_POST['port'];
                } ?>" placeholder="e.g. 12360">
            </div>
        </div>
        <div class="control-group">
            <label class="control-label">LAN or NET</label>
            <div class="controls">
                <select name="mode" class="input-small"><option value="lan" <?php if (! empty($_POST['mode']) && $_POST['mode'] == 'lan') {
                    echo 'selected';
                } ?>>LAN</option><option value="net" <?php if (! empty($_POST['mode']) && $_POST['mode'] == 'net') {
                    echo 'selected';
                } ?>>NET</option></select>
