...
<form class="form-horizontal" action="<?php echo $_SERVER['PHP_SELF'] ?>?step=1" method="POST">
    <div class="control-group">
        ...
        <div class="controls">
            <input type="text" id="address" name="address" value="<?php if (! empty($_POST['address'])) {
                echo $_POST['address'];
            } ?>">
        </div>
    </div>
    ...
    <div class="control-group">
        ...
        <div class="controls">
            <input type="text" id="database" name="database" value="<?php if (! empty($_POST['database'])) {
                echo $_POST['database'];
            } ?>">
        </div>
    </div>
...
if (! empty($_POST['address']) && ! empty($_POST['port']) && ! empty($_POST['username']) && ! empty($_POST['password']) && ! empty($_POST['database'])) {
    if ($error == 'connected') {
        $_SESSION['mysql_connection'] = ['address' => $_POST['address'], 'port' => $_POST['port'], 'username' => $_POST['username'], 'password' => $_POST['password'], 'database' => $_POST['database'], 'status' => 'pending'];
        ?>
    ...
    param:
        dsn: mysql:host=<?php echo $_POST['address'];
        if ($_POST['port'] != '3306') {
            echo ':'.$_POST['port'];
        } ?>;dbname=<?php echo $_POST['database']; ?>
