<x-layouts.admin :title="__('Edit Config')">
    <div class="space-y-6">
        {{-- Header --}}
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-xl font-bold text-white">{{ __('Edit Config') }}</h1>
                <p class="mt-1 text-sm text-gray-400">{{ $config->name }}</p>
            </div>
        ...
        <form method="POST" action="{{ route('admin.configs.update', $config) }}" class="space-y-6">
            @csrf
            @method('PUT')
