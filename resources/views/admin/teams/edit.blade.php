<x-layouts.admin :title="__('Edit Team')">
    <div class="space-y-6">
        {{-- Header --}}
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-xl font-bold text-white">{{ __('Edit Team') }}</h1>
                <p class="mt-1 text-sm text-gray-400">{{ $team->name }}</p>
            </div>
        ...
        <form method="POST" action="{{ route('admin.teams.update', $team) }}" class="space-y-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" id="shorthandle" name="shorthandle"
                               value="{{ old('shorthandle', $team->shorthandle) }}"
                               maxlength="10"
            ...
            @if($seasons->isNotEmpty())
                <div class="bg-gray-800/50 border border-gray-700 rounded-xl p-6 space-y-4">
                    ...
                    @php
                        $teamSeasonIds = old('seasons', $team->seasons->pluck('id')->toArray());
                    @endphp
                        ...
                        @foreach($seasons as $season)
                            <label class="flex items-center gap-3 cursor-pointer rounded-lg bg-gray-700/50 border border-gray-600 px-3 py-2 hover:border-yellow-600/50">
                                <input type="checkbox"
                                       name="seasons[]"
                                       value="{{ $season->id }}"
                                       {{ in_array($season->id, $teamSeasonIds) ? 'checked' : '' }}
