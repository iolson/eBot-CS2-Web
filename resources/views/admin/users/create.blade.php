<x-layouts.admin :title="__('Create User')">
    <div class="space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-xl font-bold text-white">{{ __('Create User') }}</h1>
                <p class="mt-1 text-sm text-gray-400">{{ __('Add a new admin panel user.') }}</p>
            </div>
        ...
        <form method="POST" action="{{ route('admin.users.store') }}" class="space-y-6">
            ...
            {{-- Password --}}
            <div class="bg-gray-800/50 border border-gray-700 rounded-xl p-6 space-y-4">
                <h2 class="text-sm font-semibold uppercase tracking-wider text-gray-400">{{ __('Password') }}</h2>
                    ...
                    <div>
                        <label class="flex items-center gap-3 cursor-pointer">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500">{{ __('Inactive users cannot log in.') }}</p>
                            </div>
                        </label>
                        @error('is_active')
                            <p class="mt-1 text-xs text-red-400">{{ $message }}</p>
