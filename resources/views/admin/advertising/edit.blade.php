<x-layouts.admin :title="__('Edit Ad')">
    <div class="space-y-6">
        {{-- Header --}}
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-xl font-bold text-white">{{ __('Edit Ad') }}</h1>
                <p class="mt-1 text-sm text-gray-400">{{ \Illuminate\Support\Str::limit($advertising->message, 60) }}</p>
            </div>
        ...
        <form method="POST" action="{{ route('admin.advertising.update', $advertising) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="bg-gray-800/50 border border-gray-700 rounded-xl p-6 space-y-4">
                ...
                <div>
                    ...
                    <select id="season_id" name="season_id"
                        ...
                        <option value="">{{ __('All Seasons / Global') }}</option>
                        @foreach($seasons as $season)
                            <option value="{{ $season->id }}"
                                {{ old('season_id', $advertising->season_id) == $season->id ? 'selected' : '' }}>
                                {{ $season->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <textarea id="message" name="message"
                              ...
                              required>{{ old('message', $advertising->message) }}</textarea>
                    @error('message')
                        <p class="mt-1 text-xs text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <label class="flex items-center gap-3 cursor-pointer">
                        <input type="checkbox" name="active" value="1"
                               {{ old('active', $advertising->active) ? 'checked' : '' }}
