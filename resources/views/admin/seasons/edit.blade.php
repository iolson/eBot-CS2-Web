<x-layouts.admin :title="__('Edit Season')">
    <div class="space-y-6">
        {{-- Header --}}
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-xl font-bold text-white">{{ __('Edit Season') }}</h1>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="date" id="start" name="start"
                               value="{{ old('start', $season->start ? \Carbon\Carbon::parse($season->start)->format('Y-m-d') : '') }}"
                               class="mt-1 block w-full rounded-lg bg-gray-700 border border-gray-600 text-white px-4 py-2.5 focus:border-yellow-500 focus:outline-none">
                        @error('start')
                            <p class="mt-1 text-xs text-red-400">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="date" id="end" name="end"
                               value="{{ old('end', $season->end ? \Carbon\Carbon::parse($season->end)->format('Y-m-d') : '') }}"
                               class="mt-1 block w-full rounded-lg bg-gray-700 border border-gray-600 text-white px-4 py-2.5 focus:border-yellow-500 focus:outline-none">
                        @error('end')
                            <p class="mt-1 text-xs text-red-400">{{ $message }}</p>
                        @enderror
