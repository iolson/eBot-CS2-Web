<x-layouts.app :title="__('Matches')">
    <div class="space-y-4">
        ...
        {{-- Season filter --}}
        @if(session('selected_season_id'))
            <div class="flex items-center gap-3">
                <span class="text-sm text-gray-400">{{ __('Filtered by season') }}</span>
                <a href="{{ route('seasons.index') }}" class="text-xs text-yellow-400 hover:text-yellow-300">{{ __('Change') }}</a>
            </div>
        @endif
