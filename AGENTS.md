...
This file provides guidance to AI coding agents (Claude Code, Gemini CLI, etc.) working in this repository.
...
- **Blade + Alpine.js** for server-rendered UI (no Livewire)
...
- `app/` — Laravel application (Models, Services, Http Controllers, Console Commands)
...
- `routes/web.php` — All routes (public frontend, admin, JSON exports)
...
Key eBot settings: `EBOT_IP`, `EBOT_PORT`, `EBOT_WEBSOCKET_URL`, `EBOT_WEBSOCKET_SECRET_KEY`, `EBOT_DEMO_PATH`, `EBOT_DEMO_DOWNLOAD`, `EBOT_MODE`.
...
- Feature tests for all HTTP endpoints and Blade views
- Compatibility tests verifying JWT/AES output matches eBot Node.js expectations
- Test coverage target: 80%+ (requires Xdebug: `XDEBUG_MODE=coverage ./vendor/bin/pest --coverage-text`)
- Current test count: 268 tests, 572 assertions (Phase 13); grows with each phase
